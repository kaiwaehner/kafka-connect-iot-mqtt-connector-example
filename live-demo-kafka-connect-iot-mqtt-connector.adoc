...
Here is the same example with an additional Single Message Transform SMT) to route specific content to another topic:
                ...
                "config" : {
                    ...
                    "transforms.dropPrefix.regex":"fraud-(.*)",
                    "transforms.dropPrefix.replacement":"fraud"
                }
                }'
